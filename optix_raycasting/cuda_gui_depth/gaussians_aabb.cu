...
static __forceinline__ __device__ void computeBufferForward(const unsigned int idx_ray,const unsigned int num_sh, const unsigned int p0, const float dt, const float tbuffer,
    ...
    float* density_buffer){
        ...
        for (int sphere_iter=0;sphere_iter<p0; sphere_iter++){
            // int primitive_index= __float_as_int(params.particle_data[idx_ray * params.max_sphere_slice + sphere_iter].y);     
            int primitive_index= params.hit_sphere_idx[idx_ray * params.max_sphere_slice + sphere_iter];       
...
static __forceinline__ __device__ void colorBlending(const unsigned int idx_ray,const unsigned int num_sh, const unsigned int p0, const float dt, const float tbuffer,
    const float3 ray_origin, const float3 ray_direction,
    float* density_buffer,float &ray_depth, float &transmittance)
{
        ...
        for(int index_buffer=0; index_buffer<BUFFER_SIZE; index_buffer++){
            ...
            //     continue;
            // }
            ray_depth += transmittance * alpha * t_sample;
...
extern "C" __global__ void __raygen__rg()
{
    ...
    if(tenter<texit){
        ...
        float3 depth_buffer[BUFFER_SIZE]={make_float3(0.0f)};
        colorBlending(idx_ray_flatten,num_sh, p0, dt, tbuffer,  
            ray_origin, ray_direction, 
            density_buffer,ray_depth, transmittance );

        }
        tbuffer+=slab_spacing;
        }
        // float3 bg_color = make_float3(1.0f, 1.0f, 1.0f);
        // ray_depth += transmittance * bg_color;
        ...
        // float3 bg_color = make_float3(1.0f, 1.0f, 1.0f);
        params.depth_buffer[idx_ray.y * params.width + idx_ray.x] = 0.0f;
    }
}
...
extern "C" __global__ void __anyhit__ah() {
    const unsigned int num_spheres = optixGetPayload_0();

    if (num_spheres >= params.max_sphere_slice) {
    ...
    params.hit_sphere_idx[idx_ray_flatten * params.max_sphere_slice + num_spheres] = current_sphere_idx;

    // params.particle_data[idx_ray * params.max_ray_spheres + num_spheres].x=optixGetRayTmax();
    // params.particle_data[idx_ray * params.max_ray_spheres + num_spheres].x=length(optixGetWorldRayOrigin()-params.positions[current_sphere_idx]);
    // params.particle_data[idx_ray * params.max_ray_spheres + num_spheres].y=__int_as_float(current_sphere_idx);
    // params.sphere_idx[idx_ray * params.max_ray_spheres + num_spheres]=current_sphere_idx;

    optixSetPayload_0(num_spheres + 1);
